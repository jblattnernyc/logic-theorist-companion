...
- Added a companion `.gitignore` covering local system files, Python cache files, virtual environments, temporary files, incidental logs, and Common Lisp compiled artifacts while preserving Markdown, CSV, JSON, manifest files, and curated logs under `runs/` and `evidence/` by default.
...
- Review the local companion scaffold before any commit or push.
- Decide whether to commit and push the companion scaffold to `jblattnernyc/logic-theorist-companion`.
